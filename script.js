...
window.onload = function () {
  ...
  encodeBtn.onclick = function () {
    ...
    if (encodingType === "Run-length encoding (RLE)") {
      console.log("encode onclick RLE");
      var uploadedFile = uploadFile.files[0];
      if (uploadedFile === undefined) {
      ...
      fileReader.onload = function (fileLoadedEvent) {
        ...
        let [encodedString, outputMsg] = compressRLE(text);
        myDownloadFile(
...
function selectItem(element) {
  document.getElementById("selectedItem").innerText = element.innerText;
}
